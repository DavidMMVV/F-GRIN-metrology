...
def get_prop_data_beams(sim_shape: tuple[int,int,int] | list[int], 
    ...
    Z = torch.arange(sim_shape[0], device=DEVICE) * dz
    ...
    n_vol = torch.as_tensor(beam.get_n()[None].repeat((sim_shape[0],1,1)) 
             ...
             else beam.get_n(Z)[:,None, None].repeat((1, sim_shape[1], sim_shape[2])), dtype=torch.float64, device=DEVICE)

    i_sampled = torch.linspace(0, sim_shape[0]-1, steps=n_samples, device=DEVICE).long()
    i_prev = 0
    Ui = beam.get_E(0)
    L_arr_mod = torch.zeros(i_sampled.shape, dtype=torch.float64, device=DEVICE)
    L_arr_phase = torch.zeros(i_sampled.shape, dtype=torch.float64, device=DEVICE)
